...
class _DataMigrationPageState extends State<DataMigrationPage> {
      ...
      if (playlistIdsJson != null) {
        try {
          ...
          for (var id in playlistIds) {
            if (id is String) {
              exportData['playlist_info_$id'] = prefs.getString('playlist_info_$id');
              exportData['playlist_songs_$id'] = prefs.getString('playlist_songs_$id');
            }
          }
        } catch (e) {
  ...
  Map<String, dynamic> _exportSettings(SharedPreferences prefs) {
    ...
    settings[SettingsManager.KEY_VERSION_CODE] = SettingsManager.DEFAULT_VERSION_CODE;
  ...
  void _downloadWebFile(String content) {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text('Web端文件下载功能待完善')),
    );
  }
  ...
  Future<void> _saveToFile(String content) async {
    try {
      String fileName = 'bilimusic_data_${DateTime.now().millisecondsSinceEpoch}.json';
    ...
    try {
      ...
      if (result != null) {
          ...
          final shouldContinue = await showDialog<bool>(
            ...
            builder: (context) {
              return AlertDialog(
                ...
                title: Text('版本信息缺失'),
                content: Text('导入的数据文件中不包含版本信息，可能存在兼容性问题，请确认是否选择了正确的数据文件。'
          ...
          final shouldContinue = await showDialog<bool>(
            ...
            builder: (context) {
              return AlertDialog(
                icon: Icon(Icons.warning),
                title: Text('数据版本不兼容'),
                content: Text('导入的数据文件版本为 $dataVersionCode，'
        ...
        importData.forEach((key, value) {
          if (key.startsWith('playlist_info_') || key.startsWith('playlist_songs_')) {
            if (value != null) {
        ...
        if (mounted) {
          ...
          if (shouldRestart == true) {
            // 这里可以添加重启应用的逻辑
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(content: Text('请手动重启应用以应用更改')),
            );
          }
        }
      } else {
